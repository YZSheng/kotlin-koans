...
private operator fun MyDate.plus(period: TimeInterval): MyDate = this.addTimeIntervals(period, 1)

fun task29_2(today: MyDate): MyDate = today + YEAR * 2 + WEEK * 3 + DAY * 5

private operator fun MyDate.plus(interval: MultiplesOfTimeInterval): MyDate {
    return this.addTimeIntervals(interval.timeInterval, interval.multiples)
}

private operator fun TimeInterval.times(i: Int): MultiplesOfTimeInterval = MultiplesOfTimeInterval(this, i)

data class MultiplesOfTimeInterval(val timeInterval: TimeInterval, val multiples: Int)
